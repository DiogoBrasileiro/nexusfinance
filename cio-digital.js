...
var AGENTS = {
  MACRO_ORACLE: { desc: 'Analisa o cenario global — juros, dolar, commodities.', styles: ['Druckenmiller','Dalio','Soros'], pipeline: ['daily','deep'] },
  ...
  EQUITY_STOCK_MASTER: { desc: 'Analisa setores da bolsa — onde ha oportunidade.', styles: ['Lynch','Fisher','Smith'], pipeline: ['deep'] },
...
function renderMobileNav() {
  var items = [
    ...
    { id: 'opportunities', icon: '🎯', label: 'Acoes' },
...
function renderLogin() {
  ...
  note.textContent = 'Dados de mercado ao vivo via Binance, BCB e outras fontes. Conteudo educacional — nao constitui recomendacao individual.';
...
function renderSidebar() {
  ...
  var verEl = div('sb-ver', 'v' + APP_VERSION + ' • Dados Reais');
  ...
  var NAV_SECTIONS = [
    { title: 'Painel', items: [
      ...
      { id:'mercado',        icon:'📊', label:'Cotacoes ao Vivo' },
  ...
  if (!hasKey) {
    ...
    nokey.appendChild(div('nokey-sub', 'Gemini e gratuito — clique aqui'));
...
function renderDashboard() {
  ...
  append(pipeToggle,
    el('button', { class:'pipe-btn'+(ST.pipeline==='daily'?' on':''), onclick:function(){setState({pipeline:'daily'});} }, '⚡ Rapido (4 agentes)'),
    el('button', { class:'pipe-btn'+(ST.pipeline==='deep'?' on':''), onclick:function(){setState({pipeline:'deep'});} }, '🔬 Completo (10 agentes)')
  );
  ...
  if (!ST.brief) {
    ...
    em.innerHTML = '<div class="empty-icon">⚡</div><div class="empty-title">Pronto para analisar</div><div class="empty-sub">Selecione seu perfil acima e clique em "Gerar Plano do Dia" para receber instrucoes claras: o que comprar, vender ou esperar hoje.</div>';
    wrap.appendChild(em);
  ...
  if (ST.brief._error) {
    wrap.appendChild(div('banner banner-err', '⚠ Erro ao processar resposta da IA. Tente novamente ou verifique "Outputs IA".'));
    wrap.appendChild(renderDisclaimer());
    return wrap;
  }
...
function renderMercado() {
  ...
  var hdr = div('page-header');
  var titleSide = div('');
  titleSide.appendChild(div('page-title', '📊 COTACOES AO VIVO'));
  var sub = div('page-sub');
  sub.textContent = ST.snapshot ? 'Atualizado: ' + (ST.snapshot.quality.fetched_at || '') + ' • Fontes: ' + (ST.snapshot.quality.source || '') : 'Nao carregado';
  ...
  if (!ST.snapshot || !Object.keys(ST.snapshot.prices || {}).length) {
    var em = div('empty-state');
    em.innerHTML = '<div class="empty-icon">📡</div><div class="empty-title">Nenhum dado carregado</div><div class="empty-sub">Clique em "Atualizar Agora" para buscar cotacoes de Binance, BCB e outras fontes.</div>';
    wrap.appendChild(em);
    return wrap;
  }
...
function renderOpportunities() {
  ...
  if (!opps.length) {
    var em = div('empty-state');
    em.innerHTML = '<div class="empty-icon">🎯</div><div class="empty-title">Gere o Plano do Dia primeiro</div><div class="empty-sub">Va em "Morning Brief" e clique em "Gerar Plano do Dia" para receber instrucoes personalizadas.</div>';
    wrap.appendChild(em);
  } else {
...
function renderDisclaimer() {
  ...
  d.innerHTML = '⚠️ <strong>Conteudo educacional.</strong> Nao constitui recomendacao individual. Todo investimento tem risco. Decisoes sao de exclusiva responsabilidade do investidor.';
  return d;
}
...
function renderAIKey() {
  ...
  t.appendChild(div('page-sub', 'Configure o cerebro de IA — Gemini e GRATUITO'));
  hdr.appendChild(t);
  if (hasKey) {
  ...
  hero.innerHTML = '<div class="key-hero-icon">🔑</div><div class="key-hero-title">Conecte sua IA</div><div class="key-hero-sub">Escolha o provedor e cole sua chave. O Gemini do Google e 100% gratuito e funciona perfeitamente para analise financeira.</div>';
  wrap.appendChild(hero);

  var PROVIDERS = {
    anthropic: { name:'Anthropic Claude', icon:'🧠', desc:'Alta qualidade para analise. Pago.', link:'https://console.anthropic.com/', models:[{id:'claude-sonnet-4-20250514',label:'Claude Sonnet 4 (recomendado)'},{id:'claude-haiku-4-5-20251001',label:'Claude Haiku 4.5 (economico)'}] },
    ...
    gemini:    { name:'Google Gemini ⭐ GRATIS', icon:'♊', desc:'Gemini 2.0 Flash — GRATUITO. Tier livre generoso, sem cartao. Perfeito para comecar.', link:'https://aistudio.google.com/app/apikey', models:[{id:'gemini-2.0-flash',label:'Gemini 2.0 Flash (gratis - recomendado)'},{id:'gemini-2.0-flash-lite',label:'Gemini 2.0 Flash Lite (mais leve)'},{id:'gemini-1.5-pro',label:'Gemini 1.5 Pro (pago)'}] }
  };
  ...
  var testBtn = btn('btn-primary', '▶ Testar Conexao', function() {
    ...
    callAI('Responda apenas com JSON valido.', '{"ping":true}', 50).then(function() {
      msgEl.className='banner banner-ok'; msgEl.textContent='✓ Chave valida! IA conectada com ' + (ST.aiCfg.provider==='gemini'?'Gemini Free':ST.aiCfg.provider==='openai'?'OpenAI':'Anthropic');
    }).catch(function(e) {
      AIKEY.set(prevKey); msgEl.className='banner banner-err'; msgEl.textContent='✗ Erro: ' + e.message;
    }).then(function() { msgEl.style.display='flex'; testBtn.disabled=false; testBtn.textContent='▶ Testar Conexao'; });
  });
...
function renderSupabase() {
  ...
  [['config','⚙ Config'],['schema','🗃 SQL'],['history','📂 Historico']].forEach(function(tab) {
  ...
  if (ST.sbTab === 'config') {
    ...
    desc.textContent = 'Opcional — para salvar historico dos planos gerados. Acesse app.supabase.com → projeto → Settings → API.';
...
function renderLogs() {
  ...
  var hdr = div('page-header');
  var t = div('');
  t.appendChild(div('page-title', '📋 HISTORICO'));
...
function renderSettings() {
  ...
  var hdr = div('page-header');
  var t = div('');
  t.appendChild(div('page-title', '⚙️ CONFIGURACOES'));
