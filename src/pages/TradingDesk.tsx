          ...
          {(currentMasterPlan || currentFactCheck) && (
            <section id="estrategia-indicada" className="bg-[var(--color-surface)] rounded-[var(--radius-card)] p-6 shadow-[var(--shadow-glow)] border border-[var(--color-primary)]/30 relative overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-[var(--color-surface-elevated)] p-4 rounded-xl border border-[var(--color-border-subtle)] text-center relative overflow-hidden">
                  <span className="block text-xs font-bold text-[var(--color-text-muted)] mb-1 uppercase tracking-wider">Entrada</span>
                  <span className="block text-2xl font-mono font-bold text-[var(--color-text-main)]">
                    {currentMasterPlan?.posture === 'ESPERAR' || !currentMasterPlan?.entry.entry_price ? '—' : `$${currentMasterPlan.entry.entry_price.toLocaleString()}`}
                  </span>
                </div>
                <div className="bg-[var(--color-primary-glow)] p-4 rounded-xl border border-[var(--color-primary)]/30 text-center relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-b from-[var(--color-primary)]/5 to-transparent"></div>
                  <span className="block text-xs font-bold text-[var(--color-primary)] mb-1 uppercase tracking-wider relative z-10">Alvo (+{currentMasterPlan?.targets.profit_target_pct || targetPct}%)</span>
                  <span className="block text-2xl font-mono font-bold text-[var(--color-primary)] drop-shadow-[0_0_8px_var(--color-primary-glow)] relative z-10">
                ...
                <div className="bg-[var(--color-error)]/10 p-4 rounded-xl border border-[var(--color-error)]/30 text-center relative overflow-hidden">
                  <span className="block text-xs font-bold text-[var(--color-error)] mb-1 uppercase tracking-wider">Stop (-{currentMasterPlan?.targets.stop_loss_pct || stopPct}%)</span>
                  <span className="block text-2xl font-mono font-bold text-[var(--color-error)]">
              ...
              {currentFactCheck && (
                <div className={clsx(
                  ...
                  {currentFactCheck.critical_issues.length > 0 && (
                    <div className="text-xs text-[var(--color-error)] bg-[var(--color-error)]/5 p-3 rounded-lg border border-[var(--color-error)]/20 mt-2">
                      <span className="font-bold block mb-2 uppercase tracking-wider">Problemas Críticos:</span>
                      <ul className="list-disc list-inside space-y-1">
                        {currentFactCheck.critical_issues.map((issue, i) => (
                          <li key={i}>[{issue.type}] {issue.message}</li>
                        ))}
                      </ul>
                    </div>
                  )}
